...
export default {
    add: async (req, res, next) => {
        ...
        try {
            const res0=await models.Articulo.findOne({codigo:codigo});
            if(res0){
                return res.status(400).json({msj:'Ya existe el codigo'});
            }
    ...
    queryCodigo: async (req, res, next) => {
        try {
            const reg = await models.Articulo.findOne({ codigo: req.query.codigo,estado:1 })
                .populate('categoria', { nombre: 1 });
            if (!reg) {
                res.status(404).send({
                    msj: 'El articulo no se encuentra'
                });
            } else {
                res.status(200).json(reg);
            }
    ...
    update: async (req, res, next) => {
        try {
            const reg = await models.Articulo.findByIdAndUpdate({ _id: req.body._id }, {
                ...
                nombre: req.body.nombre, descripcion: req.body.descripcion, precio_venta: req.body.precio_venta, stock: req.body.stock
            });
            res.status(200).json(reg);
        } catch (e) {
            res.status(500).send({
